...
## Requesting content migration

If you have an existing site running on a non-SilverStripe CMS, you probably want to keep your existing content. There
are several ways to import the content into a site running on CWP. The easiest is to request a content migration ticket
...
to be installed. In this case, it can be easier and more secure to provide a DVD of the website code, assets and
database which can be set up on a development environment.
...
If you are importing based on taxonomy then you select a taxonomy to import. The hierarchy of terms will be imported as
a page tree, with any nodes tagged with that term imported as child pages under it.
